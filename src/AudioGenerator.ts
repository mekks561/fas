...
export class AudioGenerator {
    ...
    public createBackgroundMusic(duration: number = 60): AudioBufferSourceNode | null {
        ...
        for (let i = 0; i < bufferSize; i++) {
            ...
            // 包络
            const envelope = beatProgress < 0.1 ? beatProgress * 10 : 
                           beatProgress > 0.9 ? (1 - beatProgress) * 10 : 1;
    ...
    private getMelodyNote(beatIndex: number): number {
        const scale = [261.63, 293.66, 329.63, 349.23, 392.00, 440.00, 493.88, 523.25]; // C大调音阶
