...
export class WeaponSystem {
  ...
  private initializeLuaSkills(): void {
    ...
    // 学习快速恢复（等级 3）
    if (this.playerLevel >= 3 && this.luaSkillBridge.learnSkill('quick_heal', this.playerLevel, this.learnedSkills)) {
      this.learnedSkills.push('quick_heal');
      console.log('[WeaponSystem] Learned: quick_heal');
    }

    // 学习强力射击（等级 5）
    if (this.playerLevel >= 5 && this.luaSkillBridge.learnSkill('power_shot', this.playerLevel, this.learnedSkills)) {
      this.learnedSkills.push('power_shot');
      console.log('[WeaponSystem] Learned: power_shot');
    }

    // 学习疾风加速（等级 5）
    if (this.playerLevel >= 5 && this.luaSkillBridge.learnSkill('speed_boost', this.playerLevel, this.learnedSkills)) {
      this.learnedSkills.push('speed_boost');
      console.log('[WeaponSystem] Learned: speed_boost');
    }

    // 学习攻击强化被动（等级 2）
    if (this.playerLevel >= 2 && this.luaSkillBridge.learnSkill('attack_boost_passive', this.playerLevel, this.learnedSkills)) {
      this.learnedSkills.push('attack_boost_passive');
      console.log('[WeaponSystem] Learned: attack_boost_passive');
    }

    // 学习散射弹幕（等级 10）
    if (this.playerLevel >= 10 && this.luaSkillBridge.learnSkill('spread_shot', this.playerLevel, this.learnedSkills)) {
      this.learnedSkills.push('spread_shot');
      console.log('[WeaponSystem] Learned: spread_shot');
    }

    // 学习终极爆发（等级 20）
    if (this.playerLevel >= 20 && this.luaSkillBridge.learnSkill('ultimate_burst', this.playerLevel, this.learnedSkills)) {
    ...
    if (this.luaSkillBridge) {
      ...
      if (result.success) {
        ...
        if (result.effects && result.effects.length > 0) {
          const damageEffect = result.effects.find((e: any) => e.type === 'damage');
          if (damageEffect) {
            console.log(`[WeaponSystem] Calculated damage: ${damageEffect.value}, Critical: ${damageEffect.isCritical}`);
          }
        }
      }
    }
  ...
  private createNormalProjectile(pos: pc.Vec3, forward: pc.Vec3, config: WeaponConfig): void {
    ...
    this.projectiles.push(new Projectile(projectile, config.damage, forward.clone().scale(config.projectileSpeed), 'normal'));
  }
  ...
  private createSpreadProjectiles(pos: pc.Vec3, forward: pc.Vec3, config: WeaponConfig): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.projectiles.push(new Projectile(projectile, config.damage, direction.clone().scale(config.projectileSpeed), 'spread'));
    }
  }
  ...
  private createLaserProjectile(pos: pc.Vec3, forward: pc.Vec3, config: WeaponConfig): void {
    ...
    this.projectiles.push(new Projectile(projectile, config.damage, forward.clone().scale(config.projectileSpeed), 'laser'));
  }
  ...
  private createMissileProjectile(pos: pc.Vec3, forward: pc.Vec3, config: WeaponConfig): void {
    ...
    flame.addComponent('particlesystem', {
      ...
      spread: 20,
      colorGraph: {
        graph: new pc.CurveSet([[1, 0.5, 0], [1, 0.8, 0.2], [0, 0, 0]], 'color')
      },
      sizeGraph: {
    ...
    this.projectiles.push(new Projectile(missile, config.damage, forward.clone().scale(config.projectileSpeed), 'missile'));
  }
  ...
  private updateCurrentWeapon(): void {
    const levelConfig = this.levelConfigs[Math.min(this.weaponLevel - 1, this.levelConfigs.length - 1)];
