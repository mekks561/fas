...
export class EnhancedPlayerShip {
  ...
  private createPlayerShip(position: pc.Vec3): pc.Entity {
    ...
    wingLeft.model!.material = material;
    ...
    wingRight.model!.material = material;
    ...
    glowMaterial.update();
    engineGlow.model!.material = glowMaterial;
  ...
  public update(dt: number, controls: { 
    left: boolean; 
    right: boolean; 
    up: boolean; 
    down: boolean; 
    ...
    const currentSpeed = controls.boost ? this.speed * 2 * speedMultiplier : this.speed * speedMultiplier;
    ...
    if (controls.up) this.entity.rigidbody!.applyForce(this.entity.forward.clone().scale(currentSpeed * dt * 100));
    if (controls.down) this.entity.rigidbody!.applyForce(this.entity.forward.clone().scale(-currentSpeed * dt * 50));
    
    this.speed = this.entity.rigidbody!.linearVelocity.length();
  ...
  private getWeaponColor(type: WeaponType): pc.Color {
    switch (type) {
      case 'primary': return new pc.Color(0, 0.8, 1);
      case 'secondary': return new pc.Color(1, 0.5, 0);
      case 'special': return new pc.Color(0.8, 0, 1);
      default: return new pc.Color(1, 1, 1);
    }
  }
  ...
  private showDamageEffect(): void {
    if (this.damageParticles) {
      this.damageParticles.particlesystem!.enabled = true;
      setTimeout(() => {
        if (this.damageParticles) {
          this.damageParticles.particlesystem!.enabled = false;
        }
